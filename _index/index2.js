...
function submitInsertForm() {
    ...
    submitingForm(proName, comment, grade, 'insert_pro.php');
}

function submitDeleteForm() {
    const commentId = document.getElementById('del').value; // 獲取要刪除的評論的 ID
    submitingForm1(commentId, 'delete_pro.php');
}
...
function submitEditForm() {
    ...
    submitingForm2(proName, comment, grade, commentId, 'edit_pro.php');
}
// #insert 
...
function searchProfessorsAndTest() {
    const searchTerm = document.getElementById('professor_search2').value;
    const xhr = new XMLHttpRequest();
    xhr.open('POST', 'search2.php', true);
    xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    xhr.onreadystatechange = function () {
        if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
            const resultsContainer = document.getElementById('search_results_container2');
